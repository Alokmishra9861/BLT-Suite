...
const getOrCreateAdminUser = async (entity) => {
  const role =
  ...
  adminUser.entityIds = Array.from(
    new Set([...(adminUser.entityIds || []).map(String), String(entity._id)]),
  );
...
const seedDemoData = async () => {
  // Ensure at least one demo entity exists, then seed each entity separately
  ...
  for (const entity of entities) {
    const adminUser = await getOrCreateAdminUser(entity);

    const customers = [
      {
        name: "Acme Trading LLC",
        email: `accounts@acmetrading.${entity.code.toLowerCase()}.local`,
        phone: "+971555100001",
      ...
      },
      {
        ...
        email: `billing@globallogistics.${entity.code.toLowerCase()}.local`,
        phone: "+971555100002",
    ...
    const acmeCustomer = seededCustomers.find(
      (customer) => customer.email === customers[0].email,
    );
    const globalCustomer = seededCustomers.find(
      (customer) => customer.email === customers[1].email,
    );
    ...
    const vendors = [
      {
        ...
        email: `procurement@alphaoffice.${entity.code.toLowerCase()}.local`,
        phone: "+971555200001",
      ...
      },
      {
        ...
        email: `billing@atlascloud.${entity.code.toLowerCase()}.local`,
        phone: "+971555200002",
    ...
    const alphaVendor = seededVendors.find(
      (vendor) => vendor.email === vendors[0].email,
    );
    const atlasVendor = seededVendors.find(
      (vendor) => vendor.email === vendors[1].email,
    );
